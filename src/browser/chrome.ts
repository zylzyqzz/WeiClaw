...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`[W] weiclaw browser profile decorated (${profile.color})`);
    } catch (err) {
  ...
  log.info(
    `[W] weiclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
